...
static const char* kFeatures[] = {
    ...
    "蓝牙键盘扫描：语音命令（self.scan_ble）或串口 BTSCAN 触发扫描配对",
    "蓝牙状态图标：状态栏 WiFi 旁显示连接/扫描/断开状态",
    "蓝牙扫描显示完整设备信息：地址类型/事件类型/服务UUID/TX功率/厂商数据",
    "蓝牙日志独立记录：键盘扫描/连接/按键事件单独保存到 SD 卡（/sdcard/logs/ble_*.txt）",
    "蓝牙键盘自动重连：开机自动连接上次配对的键盘（定向连接，不扫描）",
    "蓝牙键盘快捷键语音查询：说“键盘快捷键”播报按键功能；说“键盘连上了吗”查询连接状态；BTSTATUS 串口命令查询连接状态",
