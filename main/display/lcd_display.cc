...
void LcdDisplay::InitializeLcdThemes() {
    ...
    // 1-bit monochrome optimized theme
    // All colors are either pure black or pure white — everything else thresholds to one of them
    auto light_theme = new LvglTheme("light");
    light_theme->set_background_color(lv_color_hex(0xFFFFFF));
    light_theme->set_text_color(lv_color_hex(0x000000));
    light_theme->set_chat_background_color(lv_color_hex(0xFFFFFF));
    // Bubbles use border-width for distinction, not color
    light_theme->set_user_bubble_color(lv_color_hex(0xFFFFFF));     // white fill
    light_theme->set_assistant_bubble_color(lv_color_hex(0xFFFFFF)); // white fill
    light_theme->set_system_bubble_color(lv_color_hex(0xFFFFFF));    // white fill
    ...
    // dark theme (inverted for 1-bit: black bg, white text)
...
// SHTC3 temperature/humidity sensor read (I2C addr 0x70, shared bus with audio codecs)
static bool ReadShtc3(float& temp_c, float& humidity_pct) {
    auto& board = Board::GetInstance();
    auto i2c_bus = board.GetI2cBus();
    if (i2c_bus == nullptr) { ESP_LOGW(TAG, "SHTC3: I2C bus not available"); return false; }
    ...
    if (ret != ESP_OK) { ESP_LOGE(TAG, "SHTC3: add device failed: %s", esp_err_to_name(ret)); return false; }
    uint8_t wake_cmd[] = {0x35, 0x17};
    ret = i2c_master_transmit(dev_handle, wake_cmd, 2, -1);
    if (ret != ESP_OK) { ESP_LOGE(TAG, "SHTC3: wake failed: %s", esp_err_to_name(ret)); i2c_master_bus_rm_device(dev_handle); return false; }
    vTaskDelay(pdMS_TO_TICKS(1));
    uint8_t meas_cmd[] = {0x7C, 0xA2};
    ret = i2c_master_transmit(dev_handle, meas_cmd, 2, -1);
    if (ret != ESP_OK) { ESP_LOGE(TAG, "SHTC3: meas cmd failed: %s", esp_err_to_name(ret)); i2c_master_bus_rm_device(dev_handle); return false; }
    vTaskDelay(pdMS_TO_TICKS(20));
    uint8_t raw[6];
    ret = i2c_master_receive(dev_handle, raw, 6, -1);
...
void LcdDisplay::UpdateStatusBar(bool update_all) {
    // Call base class first: battery icons, mute icon, time clock
