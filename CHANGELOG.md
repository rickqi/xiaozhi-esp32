...
### feat
- **新增 MCP 帮助能力**：
  - `McpServer::GetToolHelp()` / `GetAllToolsHelp()`——动态生成帮助文本（与注册零漂移，直接从工具注册表读取 name/description/参数必填性）
  - 新增语音工具 `self.get_mcp_help`（参数 `tool_name` 可选）：不填 → 返回**全部 MCP 工具概览**（框架通用 + 板级自定义，自动排除用户专属工具）；填工具名 → 返回该工具**详细用法**（描述 + 参数必填/可选）
  - 用法：说"帮助"查看全部；说"帮助 扫描键盘"查某个工具怎么用
  - 功能清单（kFeatures）与 docs/usage.md 同步更新
...
### feat
- **新增语音工具 `self.get_voice_commands`**：按分类播报全部可用语音命令（6 类：蓝牙键盘 / 环境状态 / 录音 / 音乐 / 对话日志 / 系统管理），每条含用户可说出的触发短语
  - 用法：问"有哪些语音命令" / "你能做什么" / "语音功能有哪些"
  - 功能清单（kFeatures）与 docs/usage.md 同步更新
...
### feat
- **新增语音工具 `self.get_ble_keyboard_status`**：查询蓝牙键盘连接状态，播报 `已连接 <键盘名>` / `未连接` / `扫描中` / `已找到待连接键盘 <名>`——与 `self.scan_ble` 形成完整语音闭环（扫描→连接→确认状态全程语音）
  - 用法：连接后问"键盘连上了吗" / "查询键盘状态"
  - 功能清单（kFeatures）与文档同步更新
...
### feat
- **蓝牙键盘自动重连**：连接成功后键盘地址+名称持久化到 NVS；开机 8s 后**定向连接**上次键盘（绝不扫描 → 不触发"连接不可达键盘"的内存泄漏陷阱）；`AutoReconnect()` 仅对已知地址发起单次连接
- **`self.scan_ble` 语音工具增强（两段式全流程）**：
  - 阻塞等待 10s 扫描完成，返回**发现的键盘设备名称**（不再是无状态固定文本）
  - 阶段感知：已连接 → 播报键盘名；第 1 次 → "找到键盘 <名>，再说一次扫描蓝牙来连接"；第 2 次 → "正在连接 <名>" / "已连接 <名>"
- **新增语音工具 `self.get_ble_keyboard_shortcuts`**：语音问"键盘有哪些快捷键"即可播报完整快捷键表（Enter/Esc/Space/方向键/R/T/M/V/Tab/数字键 1-9）
- **新增名称捕获**：`BluetoothKeyboard` 记录 `last_scan_name_`（最近扫描）与 `connected_name_`（当前连接），供语音播报
- **`BTSTATUS` 串口命令保留**：v3.4.8 新增，查询连接状态
- 功能清单（`self.get_version_info` 语音播报）同步更新；文档 usage.md/README 更新
